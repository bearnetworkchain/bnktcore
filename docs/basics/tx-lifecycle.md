...
* [Anbnkty of a Cosmos SDK Application](./app-anbnkty.md) {prereq}
...
* `--gas-prices` specifies how much the user is willing to pay per unit of gas, which can be one or multiple denominations of tokens. For example, `--gas-prices=0.025ubnkt, 0.025upho` means the user is willing to pay 0.025ubnkt AND 0.025upho per unit of gas.
...
Users of the application `app` can enter the following command into their CLI to generate a transaction to send 1000ubnkt from a `senderAddress` to a `recipientAddress`. It specifies how much gas they are willing to pay: an automatic estimate scaled up by 1.5 times, with a gas price of 0.025ubnkt per unit gas.

```bash
appd tx send <recipientAddress> 1000ubnkt --from <senderAddress> --gas auto --gas-adjustment 1.5 --gas-prices 0.025ubnkt
```
...
[`BeginBlock`](./app-anbnkty.md#beginblocker-and-endblocker), `DeliverTx` for each transaction,
and [`EndBlock`](./app-anbnkty.md#beginblocker-and-endblocker). While each full-node runs everything
...
* **Decoding:** Since `DeliverTx` is an ABCI call, `Tx` is received in the encoded `[]byte` form.
  Nodes first unmarshal the transaction, using the [`TxConfig`](./app-anbnkty#register-codec) defined in the app, then call `runTx` in `runTxModeDeliver`, which is very similar to `CheckTx` but also executes and writes state changes.
