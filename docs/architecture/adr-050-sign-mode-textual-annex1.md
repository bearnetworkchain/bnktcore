...
- Amounts are converted to `display` denom amounts and rendered as `number`s above
  - We do not change the capitalization of the denom. In practice, `display` denoms are stored in lowercase in state (e.g. `10 bnkt`), however they are often showed in UPPERCASE in everyday life (e.g. `10 BNKT`). Value renderers keep the case used in state, but we may recommend chains changing the denom metadata to be uppercase for better user display.
- One space between the denom and amount (e.g. `10 bnkt`).
- In the future, IBC denoms could maybe be converted to DID/IIDs, if we can find a robust way for doing this (ex. `cosmos:cosmos:hub:bank:denom:bnkt`)

#### Examples

- `1000000000ubnkt` -> `["1'000 bnkt"]`, because bnkt is the metadata's display denom.
...
- `["3cosm", "2000000ubnkt"]` -> `2 bnkt, 3 COSM` (assuming the display denoms are `bnkt` and `COSM`)
- `["10bnkt", "20Acoin"]` -> `20 Acoin, 10 bnkt` (assuming the display denoms are `bnkt` and `Acoin`)
