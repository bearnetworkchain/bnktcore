...
func (suite *IntegrationTestSuite) QueryDenomsMetadataRequest() {
	...
	testCases := []struct {
		...
		},
		{
			"success",
			func() {
				metadataBnkt := types.Metadata{
					Description: "The native staking token of the Cosmos Hub.",
					DenomUnits: []*types.DenomUnit{
						{
							Denom:    "ubnkt",
							Exponent: 0,
							Aliases:  []string{"microbnkt"},
						},
						{
							Denom:    "bnkt",
							Exponent: 6,
				...
				suite.app.BankKeeper.SetDenomMetaData(suite.ctx, metadataBnkt)
				suite.app.BankKeeper.SetDenomMetaData(suite.ctx, metadataEth)
				expMetadata = []types.Metadata{metadataBnkt, metadataEth}
				req = &types.QueryDenomsMetadataRequest{
					Pagination: &query.PageRequest{
						Limit:      7,
						CountTotal: true,
