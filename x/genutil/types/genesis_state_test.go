...
func TestGenesisStateFromGenFile(t *testing.T) {
	...
	require.Equal(t, "ubnkt", bankGenesis.DenomMetadata[0].GetBase())
	require.Equal(t, "mbnkt", bankGenesis.DenomMetadata[0].GetDenomUnits()[1].GetDenom())
	require.Equal(t, []string{"millibnkt"}, bankGenesis.DenomMetadata[0].GetDenomUnits()[1].GetAliases())
