...
func (suite *AnteTestSuite) TestAnteHandlerFees() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				err := testutil.FundAccount(suite.app.BankKeeper, suite.ctx, addr0, sdk.NewCoins(sdk.NewInt64Coin("bnkt", 149)))
				suite.Require().NoError(err)
			},
			false,
			false,
		...
		},
		{
			...
			func() {
				...
				require.True(sdk.IntEq(suite.T(), suite.app.BankKeeper.GetAllBalances(suite.ctx, addr0).AmountOf("bnkt"), sdk.NewInt(149)))

				err := testutil.FundAccount(suite.app.BankKeeper, suite.ctx, addr0, sdk.NewCoins(sdk.NewInt64Coin("bnkt", 1)))
				suite.Require().NoError(err)
			},
			false,
			true,
		...
		},
		{
			...
			func() {
				...
				require.True(sdk.IntEq(suite.T(), suite.app.BankKeeper.GetAllBalances(suite.ctx, modAcc.GetAddress()).AmountOf("bnkt"), sdk.NewInt(150)))
				require.True(sdk.IntEq(suite.T(), suite.app.BankKeeper.GetAllBalances(suite.ctx, addr0).AmountOf("bnkt"), sdk.NewInt(0)))
			},
			false,
			false,
			sdkerrors.ErrInsufficientFunds,
	...
	testCases := []TestCase{
		{
			...
			func() {
				feeAmount = sdk.NewCoins(sdk.NewInt64Coin("bnkt", 0))
				gasLimit = 0
			},
			false,
			false,
