...
func (suite *AnteTestSuite) TestEnsureMempoolFees() {
	...
	coins := sdk.NewCoins(sdk.NewCoin("bnkt", sdk.NewInt(300)))
	...
	bnktPrice := sdk.NewDecCoinFromDec("bnkt", sdk.NewDec(200).Quo(sdk.NewDec(100000)))
	highGasPrice := []sdk.DecCoin{bnktPrice}
	...
	bnktPrice = sdk.NewDecCoinFromDec("bnkt", sdk.NewDec(0).Quo(sdk.NewDec(100000)))
	lowGasPrice := []sdk.DecCoin{bnktPrice}
	...
	// of 150bnkt, the priority here is 150.
	suite.Require().Equal(feeAmount.AmountOf("bnkt").Int64(), newCtx.Priority())
}
...
func (suite *AnteTestSuite) TestDeductFees() {
	...
	coins := sdk.NewCoins(sdk.NewCoin("bnkt", sdk.NewInt(10)))
	...
	suite.app.AccountKeeper.SetAccount(suite.ctx, acc)
	err = testutil.FundAccount(suite.app.BankKeeper, suite.ctx, addr1, sdk.NewCoins(sdk.NewCoin("bnkt", sdk.NewInt(200))))
	suite.Require().NoError(err)
