...
upon receiving txGovSubmitProposal from sender do
  ...
  if (initialDeposit.Bnkts <= 0) OR (sender.BnktBalance < initialDeposit.Bnkts)
    // InitialDeposit is negative or null OR sender has insufficient funds
    throw

  if (txGovSubmitProposal.Type != ProposalTypePlainText) OR (txGovSubmitProposal.Type != ProposalTypeSoftwareUpgrade)

  sender.BnktBalance -= initialDeposit.Bnkts
...
`Proposal.TotalDeposit < ActiveParam.MinDeposit`, Bnkt holders can send
...
upon receiving txGovDeposit from sender do
  ...
  if (txGovDeposit.Deposit.Bnkts <= 0) OR (sender.BnktBalance < txGovDeposit.Deposit.Bnkts) OR (proposal.CurrentStatus != ProposalStatusOpen)
  ...
  if (CurrentBlock >= proposal.SubmitBlock + depositParam.MaxDepositPeriod)
    ...
    // sender can deposit
    sender.BnktBalance -= txGovDeposit.Deposit.Bnkts
...
bonded Bnkt holders are able to send `MsgVote` transactions to cast their
