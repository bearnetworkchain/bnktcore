...
func TestFullFundraiserPath(t *testing.T) {
	require.Equal(t, "m/44'/1230'/0'/0/0", hd.NewFundraiserParams(0, 1230, 0).String())
}

func initFundraiserTestVectors(t *testing.T) []addrData {
	// NOTE: bnkt fundraiser address
	// var hdPath string = "m/44'/1230'/0'/0/0"
...
func TestFundraiserCompatibility(t *testing.T) {
	...
	for i, d := range hdToAddrTable {
		...
		priv, err := hd.DerivePrivateKeyForPath(master, ch, "m/44'/1230'/0'/0/0")
